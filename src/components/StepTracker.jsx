...
const ResumeUpload = ({ setStarted }) => {
  ...
  // Helper to move to next step and trigger action
  const nextStep = () => {
    if (loading) return;
  ...
  return (
    <div className="w-full px-4 md:px-8 py-8 bg-gradient-to-br from-slate-900/80 to-black/70 border border-white/10 rounded-2xl backdrop-blur-xl shadow-xl space-y-10 animate-slide-up">
      ...
      <StepTracker currentStep={step} setStep={setStep} allowFutureSteps={true} />
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2 max-h-[200px] overflow-y-auto">
              {skills.map((skill, idx) => (
                <span
                  key={idx}
                  className="bg-indigo-700/80 text-white px-3 py-1 rounded-full text-sm shadow"
                >
                  {skill}
                </span>
              ))}
